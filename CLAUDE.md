...
Static HTML site using Tailwind CSS, precompiled via the Tailwind CLI into a committed static file (`public/css/tailwind.css`) — no CDN, no server-side build step. After changing Tailwind classes or the config (`tailwind.config.js`), run `npm run build:css` and commit the regenerated `public/css/tailwind.css`. All pages also share custom CSS (`public/css/styles.css`).
...
├── TODO.md                           # (not served)
├── .github/                          # (not served)
├── package.json                      # (not served) devDeps + `build:css` script
├── tailwind.config.js                # (not served) Tailwind CLI config with design tokens
├── tailwind-src/
│   └── input.css                     # (not served) `@tailwind base/components/utilities`
└── public/                           # ← DocumentRoot
    ...
    ├── sitemap.xml
    ├── css/
    │   ├── tailwind.css              # Generated by `npm run build:css` — do not hand-edit
    │   └── styles.css                # Shared custom CSS
    ├── img/
    │   └── homepage.webp             # Homepage hero image
    ├── includes/
    │   ├── nav.html                  # Shared navigation (SSI-included)
    │   └── footer.html               # Shared footer (SSI-included)
    ├── errors/
...
- Nav and footer are loaded via Apache SSI (`<!--#include virtual="/includes/nav.html" -->` etc.), not JavaScript
